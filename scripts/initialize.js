...
async function main() {
    ...
    // EntryPoint  updateChainConfigs
    {
        const _chainIds = [
            deployedAddresses["Vizing-testnet"].ChainId,
            deployedAddresses["Base-sepolia"].ChainId,
            deployedAddresses["Arbitrum-sepolia"].ChainId,
            deployedAddresses["Optimism-sepolia"].ChainId,
            deployedAddresses["Blast-testnet"].ChainId
        ];
        const Config1 = ({
            entryPoint: deployedAddresses["Vizing-testnet"].EntryPoint,
            router: deployedAddresses["Vizing-testnet"].SyncRouter
        });

        const Config2 = ({
            entryPoint: deployedAddresses["Base-sepolia"].EntryPoint,
            router: deployedAddresses["Base-sepolia"].SyncRouter
        });

        const Config3 = ({
            entryPoint: deployedAddresses["Arbitrum-sepolia"].EntryPoint,
            router: deployedAddresses["Arbitrum-sepolia"].SyncRouter
        });

        const Config4 = ({
            entryPoint: deployedAddresses["Optimism-sepolia"].EntryPoint,
            router: deployedAddresses["Optimism-sepolia"].SyncRouter
        });

        const Config5 = ({
            entryPoint: deployedAddresses["Blast-testnet"].EntryPoint,
            router: deployedAddresses["Blast-testnet"].SyncRouter
        });
        ...
        //
        // const updateChainConfigs=await EntryPoint.updateChainConfigs(
        //     _chainIds,
        //     _config
        // );
        // await updateChainConfigs.wait();

        for(let i=0;i<_chainIds.length;i++){
            const updateChainConfig=await EntryPoint.updateChainConfig(
                    _chainIds[i],
                    _config[i]
            );
            await updateChainConfig.wait();
            console.log("updateChainConfig success");
        }
    }

    // transfer owner and manager
    // {
    //     const transferOwner = await EntryPoint.transferOwnership(owner.address);
    //     await transferOwner.wait();
    //     console.log("EntryPoint transferOwnership success");
